...
def results(cvae, affinity_level, ionization_level, generate, z_dim, cond_dim, 
            atom_number, atom_labels, row_dim, col_dim, atom_decoder_m, bond_decoder_m):
    # 获取模型所在的设备
    device = next(cvae.parameters()).device
    print(f'生成设备: {device}, Affinity: {affinity_level}, Ionization: {ionization_level}')
    
    with torch.no_grad():
        # 生成随机潜在向量
        z = torch.randn(int(generate), z_dim).to(device)
        
        # 创建条件向量
        c1 = torch.zeros(int(generate), cond_dim).to(device)
        c1[:,affinity_level] = 1
        c2 = torch.zeros(int(generate), cond_dim).to(device)
        c2[:,ionization_level] = 1
            ...
            if mol is not None:
                try:
                    ...
                    if smiles and '.' not in smiles:
                        smi.append(smiles)
                        affinity.append(affinity_level)
                        ionization.append(ionization_level)
                except:
                    continue
    ...
    # 创建结果数据框
    cvae_df = pd.DataFrame({'SMILES':smi, 'Affinity':affinity, 'Ionization':ionization})
    print(f'生成了 {len(smi)} 个有效分子')
    return cvae_df
