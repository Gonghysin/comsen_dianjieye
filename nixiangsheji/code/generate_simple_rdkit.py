...
def generate_molecules_vae(model, target_properties, prop_mean, prop_std, latent_dim, num_samples=200):
    """使用VAE模型生成分子"""
    print(f"\n目标属性:")
    print(f"亲和力 (Affinity): {float(target_properties[0][0]):.4f}")
    print(f"电离能 (Ionization): {float(target_properties[0][1]):.4f}")
    ...
    for _ in range(num_samples):
        perturbation = torch.tensor([
            target_properties[0][0] * random.uniform(-0.05, 0.05),  # 减小扰动范围到±5%
            target_properties[0][1] * random.uniform(-0.05, 0.05)   # 减小扰动范围到±5%
        ], dtype=torch.float32)
        perturbed_target = target_properties[0] + perturbation
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_samples', type=int, default=3, help='生成分子的数量（默认：200）')
    parser.add_argument('--seed', type=int, default=42, help='随机种子（默认：42）')
    parser.add_argument('--model_path', type=str, default='nixiangsheji/models/best_model.pt', help='模型文件路径（默认：models/best_model.pt）')
